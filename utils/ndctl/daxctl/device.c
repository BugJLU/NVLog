		...
		const char *usage, struct daxctl_ctx *ctx)
{
	...
	case ACTION_RECONFIG:
		...
		} else if (strcmp(param.mode, "nvpc") == 0) { // XXX: nvpc
			reconfig_mode = DAXCTL_DEV_MODE_NVPC;
			if (!param.no_online) { // FIXME: nvpc cannot be online now
				fprintf(stderr,
					"--online is incompatible with --mode=nvpc, --no-online is required\n");
				rc = -EINVAL;
			}
...
static int dev_online_memory(struct daxctl_dev *dev)
{
	...
	// TODO: test if the memory is nvpc
	// if (daxctl_memory_is_nvpc(mem)) {
	// 	fprintf(stderr, "%s: failed to online memory: %s\n", devname,
	// 		"nvpc cannot be online now");
	// 	return -ENXIO;
	// }
...
static int reconfig_mode_nvpc(struct daxctl_dev *dev) // XXX: nvpc
{
	const char *devname = daxctl_dev_get_devname(dev);
	int rc, skip_enable = 0;
	...
	if (!param.no_online) {
		fprintf(stderr,
			"%s: error: memory cannot be online in nvpc, aborting\n",
			devname);
		return -EBUSY;
	}
	...
	if (!param.force && daxctl_dev_will_auto_online_memory(dev)) {
		fprintf(stderr,
			"%s: error: kernel policy will auto-online memory, but memory cannot be online in nvpc, aborting\n",
			devname);
		return -EBUSY;
	}
		...
		struct json_object **jdevs)
{
	...
	case DAXCTL_DEV_MODE_RAM:
		...
		break;
	case DAXCTL_DEV_MODE_NVPC:
		rc = reconfig_mode_nvpc(dev);
		break;
