...
DAXCTL_EXPORT int daxctl_dev_enable_nvpc(struct daxctl_dev *dev) // XXX: nvpc
{
	return daxctl_dev_enable(dev, DAXCTL_DEV_MODE_NVPC);
}
		...
		enum memory_op op, int *status)
{
	...
	// TODO: test if mem is nvpc
	// case MEM_IS_NVPC:
	// 	return memblock_is_nvpc(mem, memblock);
	}
...
DAXCTL_EXPORT int daxctl_memory_is_nvpc(struct daxctl_memory *mem) // XXX: nvpc
{
	return daxctl_memory_op(mem, MEM_IS_NVPC);
}
